...
in the inner edge of the disk near where several CBPs have been observed.  Currently, one of the most successful models in explaining the CBP population is planetary migration.  
...
causing interactions with the disk and external gas clouds, driving the black hole binary separation to decrease \citep{Roskar2015}.  Numerous studies have been 
...
has also been made to explore accretion onto binary SMBHs and how it varies with both the binary and disk properties \citep[e.g.][]{Shi2012,DOrazio2013,Nixon2013,Farris2014}.
...
The orbits of the binary stars and all disk particles were evolved using ChaNGa's implementation of a 
symplectic leapfrog integrator.  Each particle feels the force of gravity due to every other particle, including
the two central stars.  Since the orbits of the stars are not integrated using a higher order technique, I employed conservative
...
$\Delta t = \eta/\sqrt{r/a}$, $a$ is the acceleration of the
particle, $r$ is the distance to the particle or node that causes the largest
acceleration, and $\eta$ is an accuracy parameter.  I used this timestep criterion as its form is particularly well-suited for Keplerian orbits, and in the absence of collisions, it has the desirable property of giving the particle a fixed number of timesteps per orbital period \citep[$n = 2\pi/ \eta$][]{Richardson2000}.  I found that 
$\eta = 0.005$ yielded sufficiently small timesteps to properly
resolve the dynamics of the binary.  Finally, I used a Courant number of
...
The model system studied in this paper is Kepler-38, a main sequence binary composed of a G and an M dwarf.  The physical parameters for this 
...
I study the sensitivity of my results on disk resolution and aspect ratio in Simulations 9, 10, and 11, see $\S$~\ref{CBDisk:sec:VaryingDiscProps}.  A summary of the relevant simulation parameters used in this study is given in Table~\ref{tab:CBDisk_table_2}. 
...
As each simulation evolves, the time-varying gravitational force of the
...
for these simulations, a hollow central cavity forms around the secondary within the circumbinary disk.  Gaps of this size tend to remove the eccentricity-damping 
...
is present within the disk.  Since the gap opens rather quickly, in of order 5 years for the Kepler-38 binary,  this finding supports the hypothesis that the 1:3 EOLR will be the dominant resonance within the disk and will drive subsequent evolution within the system.

	%% disk Evolution Subsection %%

\subsection{Disk Evolution}

In this section, I study the structure and eccentricity of the protoplanetary disk  over $10^4$ binary periods using about 230 snapshots
for each simulation to track how the disk coevolves with the central binary.
...
the 1:3 EOLR.  To explain why the disk becomes eccentric, \citet{Papaloizou2001} showed that disk eccentricity growth occurs via a parametric instability driven by a coupling between the binary's tidal potential and a disk $m = 1$ mode that arises from some small initial disk eccentricity.  This coupling excites an $m = 2$ spiral wave from the 1:3 EOLR that removes angular momentum from the disk at constant energy causing the gas orbits to become more eccentric.  The material at the 1:3 EOLR rotates more slowly that the orbital pattern speed allowing the resonant torques to grow eccentricity in the system through the $m = 2$ wave.  I therefore expect the circumbinary disks in my simulations to become eccentric as well.  To explore this effect in my simulations, I computed the mass-averaged disk eccentricity following \citet{Pierens2007} as
...
of similar gaseous circumbinary disk simulations \citep{Kley2008,Pierens2007,Pierens2013,Farris2014}.  The disk eccentricity 
...
in $\S$~\ref{CBDisk:sec:discStructEvolution} below, suggesting that the clump impacts the disk's eccentricity modulation, but only when the binary orbit is sufficiently eccentric.  This finding is consistent with the work of \citet{Artymowicz2000} who explain that the inner disk edge precesses with a period of $10^2 - 10^3$ binary periods when $e_{disk}/e_{bin} \sim 0.2-0.7$.  

I find that the global disk eccentricity growth is predominantly due to the inner edge of the disk becoming eccentric.  Fig.~\ref{fig:CBDiskDiskEccRad} shows the disk eccentricity versus radius for Simulations 1-5 after about 520 years of evolution.  For binaries with $e_{bin} \approx 0$, disk inner edge eccentricities are of order 0.4, while more eccentric binaries tend to produce less eccentric inner disk edges.
...
The gravitational influence of the binary forces several major structural changes within the circumbinary disk.  To explore how the
structure of the disk changes with time as a function of $e_{bin}$, I examined the orbits of gas
...
within a radial distance of 3 AU from the system barycenter for
each snapshot.  I computed histograms over gas semi-major
...
relative to $\varpi$ of the binary.  Each bin is colored by the base-10 logarithm of the number of gas particles it contains.  I note that figure displays the disk in orbital element 
...
disks are rather low-mass such that disk self gravity is approximately negligible, so I expect that the gravitational influence of the binary should dominate the gas particles' evolution.  Therefore, I expect this theory to still provide a reasonable explanation for why the spiral arm remains fixed relative to the binary.  
...
of each simulation.  The eccentricity growth rate, $\dot{e}_{bin}$,
...
For all simulations, the binary semi-major axis secularly decreases where the rate of decline is lower for increasing binary orbital eccentricity as shown in Fig.~\ref{fig:CBDiskBinSemi}.  As before, the initial $e_{bin} = 0.25$ case defies this trend as it shows a greater semi-major axis decline than the initial $e_{bin} = 0.1032$ case instead of the expected lesser decline.  

The binary's longitude of periastron slowly precesses in the prograde sense at less than $1^{\circ}$yr$^{-1}$ over the duration of the simulation similar to the results of comparable simulations by \citet{Kley2015}.
...
For less eccentric binaries, the semi-major axis and eccentricity evolution is well-described by 
...
Eqn.~(\ref{CBDisk:eqn:deda}) and Eqn.~(\ref{CBDisk:eqn:deda1996}), derived by \citet{Artymowicz1996b} via examining the balance between viscous and resonant interactions at the inner 
...
To verify that Eqn.~(\ref{CBDisk:eqn:total_deda}) is a proper model for my simulations, the observed evolution of $e_{bin}$ and $a_{bin}$ for simulations with initial $e_{bin} =$ 0.05, 0.1032, and 0.25 were compared with the theoretical predictions of Eqn.~(\ref{CBDisk:eqn:total_deda}), assuming $\alpha_{eff} = 0.006$.  The comparison is shown in Fig.~\ref{fig:CBDiskFitEqn}.
...
density gradient and aspect ratio.  Here, I analyze the results of simulations that vary disk mass, simulation resolution (gas particle number), and
...
Simulation 8 with $1.5 \times M_{disk}$ where $M_{disk}$ is the fiducial disk mass used in all other simulations.  For these additional simulations, I expect the disk to be strongly coupled to the eccentric binary since the coupling only depends on the magnitude of binary eccentricity.  I also expect that the eccentricity growth will occur more quickly for systems with more 
massive disks since torque should scale with the disk mass.  The results of the simulations are shown in Fig.~\ref{fig:CBDiskVarying}.
...
To ensure that my simulations were sufficiently resolved, I ran two additional simulations, Simulations 9 and 10, in which I decreased and increased
 ...
 In both simulations, I found the general trend of eccentricity growth and semi-major axis decay held.  The lower resolution Simulation 9 eccentricity 
...
Simulations of accretion disks around binary black holes have examined the effects of large disk aspect ratios, mainly focusing these works on accretion rates in binaries.  
...
leads to increased accretion rates onto the primary star and growth of the binary mass ratio.  Simulations of disks about black hole binaries by \citet{Ragusa2016} 
showed that disks with aspect ratios H/R $\gtrsim 0.1$ have enhanced accretion rates as the inspiralling gas is not suppressed by the binary's gravitational 
torque.  To examine the disk aspect ratio's effect on disk-binary evolution in my simulations, I ran Simulation 11 with a disk aspect ratio of $H/R = 0.12$.  To 
...
binaries from these populations with periods less than about 10 days tend to be circularized.  Theoretical work on the pre-main sequence evolution of 
...
The extent to which disk-binary interactions impact astrophysical systems over the disk's lifetime is difficult to measure.  Over the course of the disk's 
...
that in principle, there is no limit to the amount of angular momentum that can be lost by a central binary to an external disk,å suggesting that binary coalescence 
...
allow the particle's eccentricity to grow at the expense of its inclination \citet{Kozai1962,Lidov1962}.  For the case of an inclined circumstellar disk about one 
...
potentially could enter the stellar Roche lobe.  Simulations of misaligned circumbinary disks by \citet{Nixon2013} showed that disks of almost all 
...
misaligned circumbinary disks, the binary eccentricity evolution is likely significantly impacted by the disk evolution and will depart from the results presented here 
...
$e_{bin}$ growth and $a_{bin}$ decay.  Faster dynamical binary evolution due to massive disks could be relevant for \kepler CBPs as the work of \citet{Dunhill2013} suggests that these CBPs formed and migrated in massive disks.  Additionally, disk-binary interactions 
...
stochastic orbital evolution \citep{Wisdom1980}.  For the case of binary orbital evolution driven by tides, \citet{Bromley2015} 
...
The location of mean motion resonances also dictate where CBPs may reside.  The numerical integrations 
...
I note, however, that $a_{bin}$ evolution appears to be a resolution dependent effect that future work should address.
...
wave within the disk that corresponded to a radially-dependent alignment of gas particle longitude of periastrons.  The spiral wave formed within 50 
...
from the system and potentially slowing down binary evolution. For simulations of gaseous circumbinary disks, I caution that the disk-binary interaction must be 
...
integrated using ChaNGa's leapfrog integrator, very conservative timestepping was employed to ensure that the binary orbit was well-resolved and 
