...
%The full sample we consider consists of the population of circumbinary planets and eclipsing binaries discovered by {\it Kepler} and is an extension of the more restricted $3 \leq P_{bin} \leq 7.45 $ days binary period regime.  

%In Fig.~\ref{fig:kepler_eb_dist}, we plot a histogram of {\it Kepler} eclipsing binaries with $P_{orb} < 10$ days and highlight in grey the period regime where binaries are likely to have a tertiary companion and in green the period region probed in this study.  Fig.~\ref{fig:kepler_eb_dist} demonstrates that there is an appreciable number of isolated binaries in our restricted sample whose lack of circumbinary planets requires explanation. 
%\begin{figure}
%	\includegraphics[width=\columnwidth]{../../Plots/kepler_eb_dist.pdf}
%    \caption{Histogram of binary orbital periods for {\it Kepler} eclipsing binaries with periods less than 10 days taken from \citet{Kirk2016}.  The grey shaded region %indicates binaries that are likely to have a tertiary companion based on observations from \citet{Tokovinin2006}.  The green shaded region represents the binary %period regime we focus on in this study.  The dash line indicates the shortest orbital period for a binary around which a circumbinary planet has been discovered.}
 %   \label{fig:kepler_eb_dist}
%\end{figure}  
 
One intriguing characteristic of the observed population of {\it Kepler} CBPs is their tendency to orbit just exterior to the dynamical stability limit \citep{Welsh2014,Winn2015}.  The dynamical stability limit, referred to as the ``critical semi-major axis" ($a_{crit}$), is the minimum semi-major axis for a CBP to remain dynamically stable \citep{Dvorak1989,Holman1999}.  \citet{Holman1999} derived and empirical formula for $a_{crit}$ from an ensemble of N-body simulations given by
\begin{equation}
...
%	\includegraphics[width=\columnwidth]{../../Plots/acrit_contour.pdf}
 %   \caption{Critical semi-major axis normalized by the binary semi-major axis according to Eq.~(\ref{eqn:crit_semi}) within the quoted uncertainty as a function of binary eccentricity and %mass ratio \citep{Holman1999}.  The critical semi-major axis peaks at large eccentricities and small mass ratios.}
...
We combine Eq.~(\ref{eqn:rot_ang_mom_dt}) and Eq.~(\ref{eqn:rot_rate_dt}) to get the net change in stellar rotation rate due to magnetic braking and stellar radius evolution under conservation of angular momentum:
...
In the case of one tidally locked star, the angular momentum of the orbit and star are explicitly coupled as any angular momentum change in the star imparts a change in the orbit as mediated by tidal forces.  In this case, we consider the following quantity
...
\subsubsection{Stellar Rotations} \label{sec:stellar_rotations}
...
\subsubsection{Stellar Radius of Gyration} \label{sec:r_g}
...
\tablecaption{N-body Simulation Initial Condition Ranges \label{tab:nbody_params}}
\tablehead{
...
\subsection{Long-term $a_{crit}$ Evolution} \label{sec:obs_acrit}
...
\subsection{Relaxed Assumptions} \label{sec:optimistic}
...
\subsection{Dynamical Stability} \label{sec:nbody_dynamics}
...
\begin{deluxetable}{l c c}
\tabletypesize{\small}
\tablecaption{Single Planet System Outcome Fraction \label{tab:nbody_single}}
\tablewidth{0pt}
\tablehead{
...
We find that in most cases in which planet b is ejected while planet c remains stable, planet b's ejection does little to impact the ``transitability" of the remaining planet.  In the case of Neptune-mass planets in Fig.~\ref{fig:nbody_transit}, the histograms for b unstable, c stable and b, c stable have similar shapes indicating that planet b's ejection does little to change planet c's FTT.  For Saturn-mass planets, specifically those with an initially larger inclination, there is a small extended tail in the FTT distribution for the case when planet b is ejected while planet c remains stable indicating that more inclined, massive planets are scattered to slightly larger inclinations and eccentricities, reducing their FTT.  
...
\begin{figure*}[t]
	\includegraphics[width=\textwidth]{nbody_transit.pdf}
...
% We anticipate that circumbinary planetary systems in which the inner-most planet falls within $a_{crit}$ due to the STEEP process will likely result in at least one planet being ejected from the system, usually the inner-most planet.
...
%A major potential shortcoming of our theory is the lack of observed circumbinary planets.  It is not certain that the 11 observed by {\it Kepler} truly represent the underlying distribution of circumbinary planets.  Even though the numerous studies referenced in $\S$~\ref{sec:intro} find the observed properties of the {\it Kepler} circumbinary planets, most notably their tendency to exist near the critical semi-major axis, to not be a selection bias, Additional detections of circumbinary planets would improve the statistics and potentially confirm or refute the current findings.  If the observed distribution of circumbinary planet orbital properties is representative of the true distribution, then our mechanism stands as an effective means to explain the lack of circumbinary planets around short-period, isolated binaries.  If not, we have at least identified a novel mechanism to destabilize the unlucky circumbinary planets that find themselves too close to the critical semi-major axis.  
